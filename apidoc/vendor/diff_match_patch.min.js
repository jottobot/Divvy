(function(){
  ...
  diff_match_patch.prototype.diff_main=function(a,b,c,d){
    'undefined'===typeof d&&(d=0>=this.Diff_Timeout?Number.MAX_VALUE:(new Date).getTime()+1E3*this.Diff_Timeout);if(null==a||null==b){
      throw Error('Null input. (diff_main)');
    }if(a==b){
      return a?[[0,a]]:[];
    }'undefined'===typeof c&&(c=!0);var e=c,f=this.diff_commonPrefix(a,b);c=a.substring(0,f);a=a.substring(f);b=b.substring(f);var f=this.diff_commonSuffix(a,b),g=a.substring(a.length-f);a=a.substring(0,a.length-f);b=b.substring(0,b.length-f);a=this.diff_compute_(a,
  ...
  diff_match_patch.prototype.diff_compute_=function(a,b,c,d){
    if(!a){
      ...
      return[[-1,a]];
    }var e=a.length>b.length?a:b,f=a.length>b.length?b:a,g=e.indexOf(f);return-1!=g?(c=[[1,e.substring(0,g)],[0,f],[1,e.substring(g+f.length)]],a.length>b.length&&(c[0][0]=c[2][0]=-1),c):1==f.length?[[-1,a],[1,b]]:(e=this.diff_halfMatch_(a,b))?(f=e[0],a=e[1],g=e[2],b=e[3],e=e[4],f=this.diff_main(f,g,c,d),c=this.diff_main(a,b,c,d),f.concat([[0,e]],c)):c&&100<a.length&&100<b.length?this.diff_lineMode_(a,b,
      d):this.diff_bisect_(a,b,d);
  };
  diff_match_patch.prototype.diff_lineMode_=function(a,b,c){
    var d=this.diff_linesToChars_(a,b);a=d.chars1;b=d.chars2;d=d.lineArray;a=this.diff_main(a,b,!1,c);this.diff_charsToLines_(a,d);this.diff_cleanupSemantic(a);a.push([0,'']);for(var e=d=b=0,f='',g='';b<a.length;){
      switch(a[b][0]){
      case 1:e++;g+=a[b][1];break;case -1:d++;f+=a[b][1];break;case 0:if(1<=d&&1<=e){
        a.splice(b-d-e,d+e);b=b-d-e;d=this.diff_main(f,g,!1,c);for(e=d.length-1;0<=e;e--){
          a.splice(b,0,d[e]);
        }b+=d.length;
      }d=e=0;g=f='';
      }b++;
  ...
  diff_match_patch.prototype.diff_bisect_=function(a,b,c){
    for(var d=a.length,e=b.length,f=Math.ceil((d+e)/2),g=f,h=2*f,j=Array(h),i=Array(h),k=0;k<h;k++){
      j[k]=-1,i[k]=-1;
    }j[g+1]=0;i[g+1]=0;for(var k=d-e,q=0!=k%2,r=0,t=0,p=0,w=0,v=0;v<f&&!((new Date).getTime()>c);v++){
      for(var n=-v+r;n<=v-t;n+=2){
        var l=g+n,m;m=n==-v||n!=v&&j[l-1]<j[l+1]?j[l+1]:j[l-1]+1;for(var s=m-n;m<d&&s<e&&a.charAt(m)==b.charAt(s);){
          m++,s++;
        }j[l]=m;if(m>d){
          t+=2;
        }else if(s>e){
...
u){
        ...
        l=g+n;u=n==-v||n!=v&&i[l-1]<i[l+1]?i[l+1]:i[l-1]+1;for(m=u-n;u<d&&m<e&&a.charAt(d-u-1)==b.charAt(e-m-1);){
          u++,m++;
        }i[l]=u;if(u>d){
          w+=2;
        }else if(m>e){
  ...
  diff_match_patch.prototype.diff_linesToChars_=function(a,b){
    function c(a){
      for(var b='',c=0,f=-1,g=d.length;f<a.length-1;){
        f=a.indexOf('\n',c);-1==f&&(f=a.length-1);var r=a.substring(c,f+1),c=f+1;(e.hasOwnProperty?e.hasOwnProperty(r):void 0!==e[r])?b+=String.fromCharCode(e[r]):(b+=String.fromCharCode(g),e[r]=g,d[g++]=r);
      }return b;
    }var d=[],e={};d[0]='';var f=c(a),g=c(b);return{chars1:f,chars2:g,lineArray:d};
  };
  ...
  diff_match_patch.prototype.diff_commonOverlap_=function(a,b){
    var c=a.length,d=b.length;if(0==c||0==d){
      return 0;
    }c>d?a=a.substring(c-d):c<d&&(b=b.substring(0,c));c=Math.min(c,d);if(a==b){
      return c;
    }for(var d=0,e=1;;){
      var f=a.substring(c-e),f=b.indexOf(f);if(-1==f){
        return d;
      }e+=f;if(0==f||a.substring(c-e)==b.substring(0,e)){
        d=e,e++;
      }
    }
  };
  diff_match_patch.prototype.diff_halfMatch_=function(a,b){
    function c(a,b,c){
      for(var d=a.substring(c,c+Math.floor(a.length/4)),e=-1,g='',h,j,n,l;-1!=(e=b.indexOf(d,e+1));){
        var m=f.diff_commonPrefix(a.substring(c),b.substring(e)),s=f.diff_commonSuffix(a.substring(0,c),b.substring(0,e));g.length<s+m&&(g=b.substring(e-s,e)+b.substring(e,e+m),h=a.substring(0,c-s),j=a.substring(c+m),n=b.substring(0,e-s),l=b.substring(e+m));
      }return 2*g.length>=a.length?[h,j,n,l,g]:null;
    }if(0>=this.Diff_Timeout){
      return null;
    }
    var d=a.length>b.length?a:b,e=a.length>b.length?b:a;if(4>d.length||2*e.length<d.length){
      return null;
    }var f=this,g=c(d,e,Math.ceil(d.length/4)),d=c(d,e,Math.ceil(d.length/2)),h;if(!g&&!d){
      return null;
    }h=d?g?g[4].length>d[4].length?g:d:d:g;var j;a.length>b.length?(g=h[0],d=h[1],e=h[2],j=h[3]):(e=h[0],j=h[1],g=h[2],d=h[3]);h=h[4];return[g,d,e,j,h];
  };
  diff_match_patch.prototype.diff_cleanupSemantic=function(a){
    ...
    }b&&this.diff_cleanupMerge(a);this.diff_cleanupSemanticLossless(a);for(f=1;f<a.length;){
      if(-1==a[f-1][0]&&1==a[f][0]){
        b=a[f-1][1];c=a[f][1];
        d=this.diff_commonOverlap_(b,c);e=this.diff_commonOverlap_(c,b);if(d>=e){
  ...
  diff_match_patch.prototype.diff_cleanupSemanticLossless=function(a){
    function b(a,b){
      ...
      if(0==a[c-1][0]&&0==a[c+1][0]){
        var d=a[c-1][1],e=a[c][1],f=a[c+1][1],g=this.diff_commonSuffix(d,e);if(g){
          ...
          var d=d+e.charAt(0),e=e.substring(1)+f.charAt(0),f=f.substring(1),k=b(d,e)+b(e,f);k>=i&&(i=k,g=d,h=e,j=f);
...
g),e=e.substring(0,e.length-g))),0===c?a.splice(b-d,c+d,[1,f]):0===d?a.splice(b-c,c+d,[-1,e]):a.splice(b-c-d,c+d,[-1,e],[1,f]),b=b-c-d+(c?1:0)+(d?1:0)+1):0!==b&&0==a[b-1][0]?(a[b-1][1]+=a[b][1],a.splice(b,1)):b++,c=d=0,f=e='';
    ...
    var c=0,d=0,e=0,f=0,g;for(g=0;g<a.length;g++){
      1!==a[g][0]&&(c+=a[g][1].length);-1!==a[g][0]&&(d+=a[g][1].length);if(c>b){
        break;
      }e=c;f=d;
  ...
  diff_match_patch.prototype.diff_prettyHtml=function(a){
    for(var b=[],c=/&/g,d=/</g,e=/>/g,f=/\n/g,g=0;g<a.length;g++){
      var h=a[g][0],j=a[g][1],j=j.replace(c,'&amp;').replace(d,'&lt;').replace(e,'&gt;').replace(f,'&para;<br>');switch(h){
      case 1:b[g]='<ins style="background:#e6ffe6;">'+j+'</ins>';break;case -1:b[g]='<del style="background:#ffe6e6;">'+j+'</del>';break;case 0:b[g]='<span>'+j+'</span>';
      }
    }return b.join('');
  };
  diff_match_patch.prototype.diff_text1=function(a){
    ...
    for(var b=0,c=0,d=0,e=0;e<a.length;e++){
      var f=a[e][0],g=a[e][1];switch(f){
      case 1:c+=g.length;break;case -1:d+=g.length;break;case 0:b+=Math.max(c,d),d=c=0;
      }
  ...
  diff_match_patch.prototype.diff_toDelta=function(a){
    for(var b=[],c=0;c<a.length;c++){
      switch(a[c][0]){
      case 1:b[c]='+'+encodeURI(a[c][1]);break;case -1:b[c]='-'+a[c][1].length;break;case 0:b[c]='='+a[c][1].length;
      }
  ...
  diff_match_patch.prototype.diff_fromDelta=function(a,b){
    for(var c=[],d=0,e=0,f=b.split(/\t/g),g=0;g<f.length;g++){
      ...
      }break;case '-':case '=':var i=parseInt(h,10);if(isNaN(i)||0>i){
        throw Error('Invalid number in diff_fromDelta: '+h);
      }h=a.substring(e,e+=i);'='==f[g].charAt(0)?c[d++]=[0,h]:c[d++]=[-1,h];break;default:if(f[g]){
...
f[g]);
    ...
    }c=Math.max(0,Math.min(c,a.length));return a==b?0:a.length?a.substring(c,c+b.length)==b?c:this.match_bitap_(a,b,c):-1;
  };
  diff_match_patch.prototype.match_bitap_=function(a,b,c){
    function d(a,d){
      var e=a/b.length,g=Math.abs(c-d);return!f.Match_Distance?g?1:e:e+g/f.Match_Distance;
    }if(b.length>this.Match_MaxBits){
      throw Error('Pattern too long for this browser.');
    }var e=this.match_alphabet_(b),f=this,g=this.Match_Threshold,h=a.indexOf(b,c);-1!=h&&(g=Math.min(d(0,h),g),h=a.lastIndexOf(b,c+b.length),-1!=h&&(g=Math.min(d(0,h),g)));for(var j=1<<b.length-1,h=-1,i,k,q=b.length+a.length,r,t=0;t<b.length;t++){
      i=0;for(k=q;i<k;){
        d(t,c+
k)<=g?i=k:q=k,k=Math.floor((q-i)/2+i);
      }q=k;i=Math.max(1,c-k+1);var p=Math.min(c+k,a.length)+b.length;k=Array(p+2);for(k[p+1]=(1<<t)-1;p>=i;p--){
        var w=e[a.charAt(p-1)];k[p]=0===t?(k[p+1]<<1|1)&w:(k[p+1]<<1|1)&w|((r[p+1]|r[p])<<1|1)|r[p+1];if(k[p]&j&&(w=d(t,p-1),w<=g)){
          if(g=w,h=p-1,h>c){
            i=Math.max(1,2*c-h);
          }else {
            break;
  ...
  diff_match_patch.prototype.patch_addContext_=function(a,b){
    if(0!=b.length){
      ...
      }d+=this.Patch_Margin;(c=b.substring(a.start2-d,a.start2))&&a.diffs.unshift([0,c]);(d=b.substring(a.start2+a.length1,a.start2+a.length1+d))&&a.diffs.push([0,d]);a.start1-=c.length;a.start2-=c.length;a.length1+=
c.length+d.length;a.length2+=c.length+d.length;
    }
  };
  diff_match_patch.prototype.patch_make=function(a,b,c){
    var d;if('string'===typeof a&&'string'===typeof b&&'undefined'===typeof c){
    ...
    if(0===b.length){
      return[];
    }c=[];a=new diff_match_patch.patch_obj;for(var e=0,f=0,g=0,h=d,j=0;j<b.length;j++){
...
e&&(this.patch_addContext_(a,h),c.push(a),a=new diff_match_patch.patch_obj,e=0,h=d,f=g);
    ...
    for(var b=[],c=0;c<a.length;c++){
      var d=a[c],e=new diff_match_patch.patch_obj;e.diffs=[];for(var f=0;f<d.diffs.length;f++){
  ...
  diff_match_patch.prototype.patch_apply=function(a,b){
    if(0==a.length){
      return[b,[]];
    }a=this.patch_deepCopy(a);var c=this.patch_addPadding(a);b=c+b+c;this.patch_splitMax(a);for(var d=0,e=[],f=0;f<a.length;f++){
      var g=a[f].start2+d,h=this.diff_text1(a[f].diffs),j,i=-1;if(h.length>this.Match_MaxBits){
  ...
  diff_match_patch.prototype.patch_addPadding=function(a){
    ...
    }var d=a[0],e=d.diffs;if(0==e.length||0!=e[0][0]){
      e.unshift([0,c]),d.start1-=b,d.start2-=b,d.length1+=b,d.length2+=b;
    }else if(b>e[0][1].length){
      var f=b-e[0][1].length;e[0][1]=c.substring(e[0][1].length)+e[0][1];d.start1-=f;d.start2-=f;d.length1+=f;d.length2+=f;
    }d=a[a.length-1];e=d.diffs;0==e.length||0!=e[e.length-1][0]?(e.push([0,
...
(h.length1+=i.length,h.length2+=i.length,0!==h.diffs.length&&0===h.diffs[h.diffs.length-1][0]?h.diffs[h.diffs.length-1][1]+=i:h.diffs.push([0,i]));j||a.splice(++c,0,h);
  ...
  diff_match_patch.prototype.patch_fromText=function(a){
    var b=[];if(!a){
      ...
      var e=a[c].match(d);if(!e){
        throw Error('Invalid patch string: '+a[c]);
      }var f=new diff_match_patch.patch_obj;b.push(f);f.start1=parseInt(e[1],10);''===e[2]?(f.start1--,f.length1=1):'0'==e[2]?f.length1=0:(f.start1--,f.length1=parseInt(e[2],10));f.start2=parseInt(e[3],10);''===e[4]?(f.start2--,f.length2=1):'0'==e[4]?f.length2=0:(f.start2--,f.length2=
parseInt(e[4],10));for(c++;c<a.length;){
        e=a[c].charAt(0);try{
  ...
  diff_match_patch.patch_obj.prototype.toString=function(){
    var a,b;a=0===this.length1?this.start1+',0':1==this.length1?this.start1+1:this.start1+1+','+this.length1;b=0===this.length2?this.start2+',0':1==this.length2?this.start2+1:this.start2+1+','+this.length2;a=['@@ -'+a+' +'+b+' @@\n'];var c;for(b=0;b<this.diffs.length;b++){
      ...
      case 1:c='+';break;case -1:c='-';break;case 0:c=' ';
