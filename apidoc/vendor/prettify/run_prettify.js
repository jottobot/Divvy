...
*/
  (function(){
    function ba(g){
      ...
      }var Y=A.addEventListener,q=!1,C=!0,x=Y?'addEventListener':'attachEvent',B=Y?'removeEventListener':'detachEvent',p=Y?'':'on';if('complete'==A.readyState){
        g.call(t,'lazy');
      }else{
...
'DOMContentLoaded',z,!1);A[x](p+'readystatechange',z,!1);t[x](p+'load',z,!1);
      ...
      var P=F[q],Z=P.src.match(/^[^?#]*\/run_prettify\.js(\?[^#]*)?(?:#.*)?$/);if(Z){
        B=Z[1]||'';P.parentNode.removeChild(P);
        break;
      }
    }var V=!0,H=[],Q=[],N=[];B.replace(/[?&]([^&=]+)=([^&]+)/g,function(g,k,z){
...
'text/css';q+1<z&&(t.error=t.onerror=function(){
            k(q+1);
      ...
      window.PR_SHOULD_USE_CONTINUATION=!0;var g;(function(){
        function k(a){
          function d(e){
            var b=e.charCodeAt(0);if(92!==b){
              return b;
            }var a=e.charAt(1);return(b=W[a])?b:'0'<=a&&'7'>=a?parseInt(e.substring(1),8):'u'===a||'x'===a?parseInt(e.substring(2),16):e.charCodeAt(1);
          }function f(e){
            if(32>e){
              return(16>e?'\\x0':'\\x')+e.toString(16);
            }e=String.fromCharCode(e);return'\\'===e||'-'===
e||']'===e||'^'===e?'\\'+e:e;
          }function b(e){
            var b=e.substring(1,e.length-1).match(/\\u[0-9A-Fa-f]{4}|\\x[0-9A-Fa-f]{2}|\\[0-3][0-7]{0,2}|\\[0-7]{1,2}|\\[\s\S]|-|[^-\\]/g);e=[];var a='^'===b[0],c=['['];a&&c.push('^');for(var a=a?1:0,h=b.length;a<h;++a){
              var l=b[a];if(/\\[bdsw]/i.test(l)){
                c.push(l);
              }else{
                var l=d(l),n;a+2<h&&'-'===b[a+1]?(n=d(b[a+2]),a+=2):n=l;e.push([l,n]);65>n||122<l||(65>n||90<l||e.push([Math.max(65,l)|32,Math.min(n,90)|32]),97>n||122<l||e.push([Math.max(97,l)&-33,Math.min(n,122)&-33]));
              }
            }e.sort(function(e,
              a){
...
c||'li'===c){
          ...
          }var b=/(?:^|\s)nocode(?:\s|$)/,g=[],k=0,I=[],m=0;f(a);return{a:g.join('').replace(/\n$/,''),c:I};
          ...
          function f(a){
            for(var m=a.i,k=a.h,c=[m,'pln'],r=0,W=a.a.match(g)||[],u={},e=0,q=W.length;e<q;++e){
              var D=W[e],w=u[D],h=void 0,l;if('string'===typeof w){
                l=!1;
              }else{
                var n=b[D.charAt(0)];if(n){
                  h=D.match(n[1]),w=n[0];
                }else{
                  for(l=0;l<p;++l){
                ...
                l=h[1];var E=D.indexOf(l),G=E+l.length;h[2]&&(G=D.length-
...
('/(?=[^/*'+b+'])(?:[^/\\x5B\\x5C'+b+']|\\x5C'+g+'|\\x5B(?:[^\\x5C\\x5D'+b+']|\\x5C'+g+')*(?:\\x5D|$))+/')+')')]);
          ...
          function b(a){
            var c=a.nodeType;if(1==c&&!k.test(a.className)){
              ...
              var d=a.nodeValue,p=d.match(q);p&&(c=d.substring(0,p.index),a.nodeValue=c,(d=d.substring(p.index+p[0].length))&&
a.parentNode.insertBefore(m.createTextNode(d),a.nextSibling),g(a),c||a.parentNode.removeChild(a));
            }
          }function g(a){
            function b(a,c){
              var d=c?a.cloneNode(!1):a,n=a.parentNode;if(n){
                var n=b(n,1),e=a.nextSibling;n.appendChild(d);for(var f=e;f;f=e){
          ...
          var d=a.j;try{
            var f=q(a.h,a.l),b=f.a;a.a=b;a.c=f.c;a.i=0;F(d,b)(a);var g=/\bMSIE\s(\d+)/.exec(navigator.userAgent),g=g&&8>=+g[1],d=/\n/g,p=a.a,k=p.length,f=0,m=a.c,t=m.length,b=0,c=a.g,r=c.length,x=0;c[r]=k;var u,e;for(e=u=0;e<r;){
            ...
            try{
              for(;b<t;){
                var l=m[b+2]||k,n=c[x+2]||k,w=Math.min(l,n),E=m[b+1],G;if(1!==E.nodeType&&(G=p.substring(f,w))){
                  g&&(G=G.replace(d,'\r'));E.nodeValue=G;var aa=E.ownerDocument,v=aa.createElement('span');v.className=c[x+1];var B=E.parentNode;B.replaceChild(v,E);v.appendChild(E);f<l&&(m[b+1]=E=aa.createTextNode(p.substring(w,l)),B.insertBefore(E,v.nextSibling));
                }f=w;f>=l&&(b+=2);f>=n&&(x+=2);
              }
            }finally{
              h&&(h.style.display=a);
            ...
            function f(){
              for(var b=R.PR_SHOULD_USE_CONTINUATION?c.now()+250:Infinity;r<p.length&&c.now()<b;r++){
                for(var d=p[r],k=h,q=d;q=q.previousSibling;){
                  var m=q.nodeType,v=(7===m||8===m)&&q.nodeValue;if(v?!/^\??prettify\b/.test(v):3!==m||/\S/.test(q.nodeValue)){
                    break;
                  }if(v){
...
!0;break;
                    ...
                    d.className+=' prettyprinted';m=k.lang;if(!m){
                      var m=q.match(t),C;!m&&(C=A(d))&&z.test(C.tagName)&&(m=C.className.match(t));m&&(m=m[1]);
                    }if(x.test(d.tagName)){
                      v=1;
                    }else {
                    ...
                    }y=k.linenums;(y='true'===y||+y)||(y=(y=q.match(/\blinenums\b(?::(\d+))?/))?y[1]&&y[1].length?+y[1]:!0:!1);y&&B(d,y,v);H({j:m,h:d,m:y,l:v,a:null,i:null,c:null,
                      g:null});
                  }
                }
            ...
            }var b=null,c=Date;c.now||(c={now:function(){
              return+new Date;
            }});var r=0,t=/\blang(?:uage)?-([\w.]+)(?!\S)/,u=/\bprettyprint\b/,e=/\bprettyprinted\b/,x=/pre|xmp/i,z=/^code$/i,w=/^(?:pre|code|xmp)$/i,h={};f();
          }},
